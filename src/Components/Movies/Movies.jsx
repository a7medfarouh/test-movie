...
export default function Movies() {
    ...
    async function getPopularMovies(currentPage) {
      let { data } = await axios.get(
        `
        https://api.themoviedb.org/3/movie/popular?api_key=44ee5523e457e74020effc2bddc4592e&language=en-US&page=${currentPage}`
      );
      setMovies(data.results);
      setLoading(false);
      // setTotal(data.total_results);
      // setVisible(data.page);
            ...
            {search.length > 0 ? (
              ...
              <>
                <div className="row py-5 my-5">
                  <h1 className="fw-bolder text-center  py-3">Popular Movies</h1>
  
                  {movies
                    .filter((ele) => ele.profile_path || ele.poster_path !== null)
                    .map((movie, index) => (
