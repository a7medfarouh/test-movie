...
export default function People() {
    ...
    async function getPopularPeople(currentPage) {
      let { data } = await axios.get(
        `
        https://api.themoviedb.org/3/person/popular?api_key=44ee5523e457e74020effc2bddc4592e&language=en-US&page=${currentPage}`
      );
            ...
            {search.length > 0 ? (
              ...
              <>
                <div className="row py-5 my-5">
                  <h1 className="fw-bolder text-center  py-3">Popular Person</h1>
  
                  {People.filter((ele) => ele.profile_path !== null).map(
                    (movie, index) => (
